...
- [ ] **Pi buttons**: a Pi `ask_user` question with three options arrives with buttons `1️⃣` to `3️⃣` and no `✏️`; pressing one answers the question in Pi (digit then `enter`, log `button sent`) and the keyboard becomes `✅ <n> · …`
...
- [ ] **Short done posts**: `Skip short done posts` → `30s`; a one-word prompt answered in 5 s → no done post, log `reason=short_turn`; a longer task posts as usual; set it back to `Off`
  ...
  - [ ] a Pi pane with `Done post` = `Reply`: the post holds Pi's last message and no summary line, the log has `reply posted` with the `~/.pi/agent/sessions/…` path
  - [ ] an agent other than Claude Code and Pi, or a Claude pane whose directory has no `~/.claude/projects/<slug>/`, still posts the screen and the log shows `reply source unavailable`
