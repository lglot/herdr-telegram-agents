...
- **Formatted**: the same reply rendered for Telegram: headings become bold,
  ...
  tables: a pipe table up to 40 characters wide becomes a monospace grid
  with aligned columns and box-drawing borders, a wider one becomes one
  `•` entry per row with every cell on its own line after its header in
  bold, so long cells wrap as prose on a phone. A fenced block never straddles two messages; long replies
